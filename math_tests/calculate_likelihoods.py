...
def calculate_likelihoods(ts_tv_file, readlen_file, taxa_file_paths, output_matrix, output_params):
    """
    ...
    init_ts_tv = pd.read_csv(ts_tv_file, names=['Read_ID', 'Taxon', 'Ts', 'Tv'], sep='\t', skiprows=[0])
    ...
    max_mismatch = round(0.05 * float(average_read_length))
    ...
    if ts_sum == 0 and tv_sum != 0:
        ...
        sigma_t = 0.05 / (float(2))
        sigma_v = 0.05 / (float(2))
        ...
        fixed_mismatch_probability = 0.05
    ...
    init_ts_tv['Likelihood'] = init_ts_tv.groupby('Read_ID')['ll_nom'].transform(lambda nom: nom / (sum(nom) + ((total_taxa_count - nom.count()) * data_ts_missing * data_tv_missing)))
    ...
    init_ts_tv['Dirichlet_Assignment'] = init_ts_tv['Likelihood'].where(init_ts_tv['Likelihood'] >= 0.95, 0.0).where(
        init_ts_tv['Likelihood'] < 0.95, 1.0)
...
if __name__ == '__main__':
    ...
    calculate_likelihoods(
        ts_tv_file='test_ts_tv_count_formatted.tsv',
        readlen_file='readlen_file.txt',
        taxa_file_paths=['Species1', 'Species2', 'Species3', 'Species4', 'Species5'],
        output_matrix='likelihood_matrix.csv',
