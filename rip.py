...
def main(args):
    ...
    if not config_yaml:
        sys.stderr.write('Error: cannot find the config yaml file {}. You definitely need one, even if it\'s empty '
    ...
    status = snakemake.snakemake(snakefile, config=config, targets=target_list, printshellcmds=True,
        ...
        touch=args.touch, forceall=args.touch, lock=args.no_lock, unlock=args.unlock)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--query_name', help='name of the query and the output directory <str>', metavar='')
    parser.add_argument('--entrez_query', help='Actual NCBI query in the NCBI query language', metavar='')
    parser.add_argument('--entrez_email', help='email address for NCBI identification', metavar='')
    parser.add_argument('--entrez_batchsize', help='batchsize for fetching records from NCBI', metavar='')
    parser.add_argument('--entrez_rank', help='Taxonomic rank to perform the identifications on', metavar='')
    parser.add_argument('--sample_name', help='Sample name prefix', metavar='')
    parser.add_argument('--sample_fastq', help='Path to the fastq input file. Can be raw or with adapters '
                                               'removed', metavar='')
    parser.add_argument('--sample_fastq_R1', help='Path to the mate 1 fastq input file, if reads are PE. '
                                                  'Can be raw or with adapters removed', metavar='')
    parser.add_argument('--sample_fastq_R2', help='Path to the mate 2 fastq input file, if reads are PE. '
                                                  'Can be raw or with adapters removed', metavar='')
    parser.add_argument('--mismatch_probability', help='base mismatch probability <float> (default: 0.05)', metavar='')
    parser.add_argument('--bowtie2_treads', help='threads for the bowtie2 alignments <int> (default: 1)', metavar='')
    parser.add_argument('--SRA_LOOKUP', help='fetch raw data files from the SRA <bool> (default: False)', metavar='')
    parser.add_argument('--PE_ANCIENT', help='treat the data as paired end end but use only COLLAPSED reads. \n'
                                             'mandatory and mutually exclusive with PE_MODERN and SE', metavar='')
    parser.add_argument('--PE_MODERN', help='treat the data as paired end end end but use only COMPLETE PAIRS of '
                                            'reads. \n '
                                            'mandatory and mutually exclusive with PE_ANCIENT and SE', metavar='')
    parser.add_argument('--SE', help='treat the data as single end end. \n'
                                     'mandatory and mutually exclusive with PE_ANCIENT and PE_MODERN', metavar='')
    ...
    parser.add_argument('--MEM_RESCALING_FACTOR', help='factor to rescale/chunk the input file for the mutlifasta '
                                                       'index for the filtering alignment (default: 2.5)', metavar='')
    parser.add_argument('--data_preprocess', action='store_true', help='Only process raw fastq files, '
                                                                       'incl downloading fastq files from the SRA, '
                                                                       'compressing them and removing any sequencing '
                                                                       'adapters  <bool> (default: False)')
    parser.add_argument('--build_db', action='store_true', help='ONly build the database from refseq and/or '
                                                                'other sequences retrieved from an NCBI query')
    parser.add_argument('--index_db', action='store_true', help='Index each taxon\'s reference genome that is '
                                                                'contained in the reference database, using bowtie2 '
                                                                ' <bool> (default: False)')
    parser.add_argument('--aln_filter_index', action='store_true', help='Build the index required for the filtering '
                                                                        'alignment step, out of all the sequences '
                                                                        'contained in the reference database '
                                                                        ' <bool> (default: False)')
    parser.add_argument('--aln_filter', action='store_true', help='Perform the filtering alignment step '
                                                                  ' <bool> (default: False)')
    parser.add_argument('--aln_meta', action='store_true', help='Perform metagenomic alignments. '
    ...
    parser.add_argument('--probabilities', action='store_true', help='Taxon assignment posterior probabilities are '
                                                                     'calculated for a given sample  <bool> '
                                                                     '(default: False)')
    parser.add_argument('--abundances', action='store_true', help='Mean posterior abundances for each taxon in our '
                                                                  'ref db, for a given sample, are calculated '
                                                                  ' <bool> (default: False)')
    parser.add_argument('--mapdamage', action='store_true', help='Performs a chemical damage analysis, '
                                                                 'by using mapDamage  <bool> (default: False)')
    parser.add_argument('--touch', action='store_true', help='Touches the timestamps of the requested target files '
                                                             'AND ALL of their dependencies  <bool> (default: False)')
    parser.add_argument('-j', '--cores', help='Number of cores for smk to use', metavar='')
    parser.add_argument('-l', '--no_lock', action='store_true', help='Apply no lock to the working directory '
                                                                     ' <bool> (default: False)')
    parser.add_argument('-ul', '--unlock', action='store_true', help='Unlock the working directory after smk is '
