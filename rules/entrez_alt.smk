...
def get_nuccore_xml(wildcards):
    ...
    """
    ncbi = NCBIRemoteProvider(email=config['entrez']['email'], keep_local=True, is_default=True)
    ...
    return ncbi.remote(input_files, db="nuccore", keep_local=True, force_overwrite=True, batchSize=20)
